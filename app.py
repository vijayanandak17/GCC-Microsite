...
def get_openai_api_key():
    """Get OpenAI API key from environment or Streamlit secrets or user input"""
    # Try to get from environment variable first
    api_key = os.getenv('OPENAI_API_KEY')
    
    # If not in environment, try Streamlit secrets (for deployment)
    if not api_key:
        try:
...
with st.sidebar:
    ...
    if analyze_button:
        if not uploaded_file:
            ...
            st.error("⚠️ Please enter your OpenAI API key or add it to .env file")
...
if not st.session_state.file_processed:
    ...
    st.markdown("""
    ### 📖 How to use:
    1. **Upload a document** (PDF, DOCX, TXT, XLS, or XLSX)
    2. **Click "Analyze Document"** to get an automated summary
    3. **Use the chat** to ask specific questions about the document
